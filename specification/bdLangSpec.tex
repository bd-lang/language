...
\usepackage{makeidx}
\makeindex
\title{Bd Programming Language Specification\\
{6th edition draft}\\
...
%   version of the specification which is purely about Bd 2.10.
...
%   superinterfaces.
% - Specify the Bd 2.0 rule that you cannot implement, extend or mix-in
%   Function.
...
% - Eliminate all references to checked and production mode, Bd 2 does
%   not have modes.
...
% - Update "FutureOr" specification for Bd 2.0.
...
% - Specify `call` for Bd 2 (no function type given to enclosing class).
...
% - Make it explicit that "dynamic" is exported from bd:core.
...
% - Integrate subtyping.md. This introduces the Bd 2 rules for subtyping,
...
\markright{Bd Programming Language Specification}
...
the Bd programming language.
It does not specify the APIs of the Bd libraries
...
\LMHash{}%
A conforming implementation of the Bd programming language
...
\item
  Bd API Reference, https://api.bdlang.org/
\end{enumerate}
...
Normative text defines the rules of Bd.
...
\item[Commentary]
  ...
  ``\commentary{%
    ...
    that the name Bd has four characters%
  }''
...
are interpreted as the names of members of the Bd core library.
...
%% cf. language issue https://github.com/bd-lang/language/issues/227.
...
of a core language which is a subset of Bd
(just slightly smaller than Bd),
and desugaring transforms any given Bd program to
...
(\commentary{that is, no Bd source code contains such an expression}).
...
\LMHash{}%
Bd is a class-based, single-inheritance, pure
object-oriented programming language.
Bd is optionally typed (\ref{types}) and supports reified generics.
...
the root of the Bd class hierarchy.

\LMHash{}%
Bd programs may be statically checked.
...
dynamic semantics of Bd as specified here, but, as mentioned,
...
\commentary{%
Bd as implemented includes extensive support for inference of omitted types.
...
\LMHash{}%
Bd programs are organized in a modular fashion into
...
\LMHash{}%
A bd program execution may occur with assertions enabled or disabled.
...
a method and a getter with the same name in Bd.
...
In Bd, if there is a lexically visible method \code{foo()},
...
\LMHash{}%
Bd supports two levels of \Index{privacy}: public and private.
...
\LMHash{}%
Bd code is always single threaded.
There is no shared-state concurrency in Bd.
...
A compile-time error must be reported by a Bd compiler
before the erroneous code is executed.

\rationale{%
A Bd implementation has considerable freedom
...
Bd is often loaded directly from source,
with no intermediate binary representation.
In the interests of rapid loading, Bd implementations
...
A Bd development environment might choose to support
...
A Bd engine runs in the context of a \Index{runtime},
...
A Bd compiler is free to report some, all,
or none of the specified static warnings before the associated code is executed.
A Bd compiler may also choose to report additional warnings
...
This is especially crucial for Bd,
...
\commentary{%
This is a change from the semantics of older versions of Bd:
...
Bd provides such a sugar in both the synchronous case,
where one returns an iterable,
and in the asynchronous case, where one returns a stream.
Bd also allows both synchronous and asynchronous functions
...
\item
  ...
  \commentary{%
  Because Bd supports dynamic function invocations,
...
\LMHash{}%
Bd allows formal parameters of instance methods,
...
that is not statically known to the Bd compiler.%
}

\commentary{%
Examples of external functions might be foreign functions
(defined in C, or Javascript etc.),
primitives of the implementation (as defined by the Bd run-time system),
...
Note that the latter cannot be written explicitly in Bd,
...
\item Every instance of type \code{Symbol}
  ...
  has primitive equality.
\item
  %% TODO(eernst): With Bd 2.15, we need to mention `List<int>` here,
...
\item
  %% TODO(eernst): With Bd 2.15, consider `f<int>`.
...
Every Bd class induces an implicit interface,
and Bd does not support specifying interfaces explicitly.
...
%% TODO(eernst): Come Bd 3.0, add 'mixin'.
the immediately enclosing class or enum.
...
\rationale{%
Inheritance of static methods has little utility in Bd.
...
Bd static methods may be seen as functions of the enclosing library.%
}
...
they are not supported as concrete syntax in a Bd program,
...
Bd subtyping is a partial pre-order,
...
%
On the other hand, it is consistent with the general property of Bd that
...
\code{C<C<C<C<$\ldots$>$\!$>$\!$>$\!$>}.
%
However, we expect that any such concept in Bd would incur a significant cost
...
This is because Bd does not, and will not, support higher-kinded types;
...
\LMHash{}%
Bd supports metadata which is used to attach
...
\LMHash{}%
An \Index{expression} is a fragment of Bd code
...
%% and serious source of ambiguity, which makes it difficult to parse Bd
...
\LMHash{}%
The predefined Bd function \code{identical()}
...
The rules for identity make it impossible for a Bd programmer
...
\commentary{%
All usages of 'constant' in Bd are associated with compile time.
...
\item
  ...
  the predefined bd function \code{identical()} discussed above
  (\ref{objectIdentity}).
  It is further a constant expression
...
\commentary{%
Integers in Bd are designed to be implemented as
64-bit two's complement integer representations.
In practice, implementations may be limited by other considerations.
For example, Bd compiled to JavaScript may use the JavaScript number type,
equivalent to Bd \code{double}, to represent integers, and if so,
...
\rationale{%
Bd also supports the operator + for string concatenation.
...
the recommended Bd idiom is to use string interpolation.%
}
...
However literal strings are already canonicalized in Bd.
...
Therefore, Bd reflection uses objects of type \code{Symbol}
rather than strings.
...
\commentary{%
Type promotion will likely get more sophisticated in a future version of Bd.
When that happens,
...
A Bd implementation may detect whether these options apply
...
%% <functionExpression>. Bd.g derives <functionExpression> from
...
and this document specifies the meaning of Bd programs
...
the \code{Isolate} class in the library \code{bd:isolate}.
...
\commentary{%
A Bd implementation will need to provide
...
a subclass of class \code{IterableBase} defined in \code{bd:core}.
The only method that needs to be added
by the Bd implementation in that case is \code{iterator}.%
}
...
a future version of Bd may allow for passing them explicitly.%
}
...
Messages are sent by invoking specific methods in the Bd libraries; there is no specific syntax for sending a message.

\commentary{%
In other words, the methods supporting sending messages
embody primitives of Bd that are not accessible to ordinary code,
...
However, Bd implementations are not required to canonicalize function objects,
...
% compilation of a complete Bd program, and then we select a suffix,
...
relative to the semantics in earlier versions of Bd,
...
\rationale{%
Built-in identifiers are identifiers that are used as keywords in Bd,
...
\LMHash{}%
A \Index{statement} is a fragment of Bd code that can be executed at run time.
...
%% normally': Cf. https://github.com/bd-lang/language/issues/139.
...
This is possible because Bd has a
...
undermining Bd's goal of familiarity.
...
is primarily making Bd a better target for code generation.%
}
...
\LMHash{}%
A Bd program consists of one or more libraries,
...
the Bd grammar does not have exactly one start symbol.
In particular, \synt{partDeclaration}
(\ref{parts})
is used in the same manner to derive the contents of a part.
There could be more, e.g.,
a hypothetical Bd REPL (read-eval-print loop) could use
\synt{expression} or \synt{statement} as a start symbol.
So there is no grammar for Bd programs as such,
only for some building blocks that are used to construct Bd programs.%
}
...
Bd's \code{pub} package management system provides a mechanism for doing so.
...
\LMHash{}%
The bd core library \code{bd:core}
is implicitly imported into every bd library other than itself
via an import clause of the form
\code{\IMPORT{} 'bd:core';}
unless the importing library explicitly imports \code{bd:core}.
Any import of \code{bd:core},
even if restricted via \SHOW, \HIDE, or \AS,
preempts the automatic import.

\rationale{%
It would be nice if there was nothing special about \code{bd:core}.
...
with names used by \code{bd:core}
...
essentially canceling \code{bd:core}'s special treatment
...
A \Index{system library} is a library that is part of the Bd implementation.
Any other library is a \Index{non-system library}.

\commentary{%
A system library can generally be recognized by having
a URI that starts with `\code{bd:}'.%
}
...
\commentary{%
A prefix namespace is not a Bd object,
...
All imports and exports of the same URI in a Bd program denotes
the same library,
...
a Bd compilation unit that should be incorporated into the current library
may be found.
...
causes the Bd system to attempt to compile the contents of
the URI that is the value of $s$.
The top-level declarations at that URI are then compiled by the Bd compiler
...
have a part identified by \code{'myPart.bd'}, but they are not the same
...
\commentary{%
A Bd program will typically be executed by executing a script.%
}
...
point of a Bd program. It is possible that this restriction will be removed
in the future.}
...
the surrounding computing environment.
For example, if Bd is running in a web browser,
...
\LMHash{}%
A URI of the form \code{bd:$s$} is interpreted as
...
\rationale{%
The intent is that, during development, Bd programmers can rely on
...
\LMHash{}%
Bd supports static typing based on interface types.
...
This choice was made deliberately during the early days of Bd
...
In recent years, Bd has evolved to have more and more static type safety,
...
\LMHash{}%
A Bd implementation must provide a static checker that detects and reports
...
successful compilation and execution of Bd code.%
}
...
\LMHash{}%
The dynamic types of a running Bd program are equivalent to
...
before generic functions were added to Bd.%
}
...
If a future version of Bd allows type arguments to be passed via the class
...
demonstrating that Bd subtyping can be decided efficiently.%
}
...
For backwards compatibility with Bd 1 programs,
...
which is declared in the library \code{bd:core},
denotes the \DYNAMIC{} type.
When the name \DYNAMIC{} exported by \code{bd:core} is evaluated
as an expression,
...
which is exported by the library \code{bd:async},
...
no invariants needed for correct execution of a Bd program
...
and because of the treatment of \VOID{} in earlier versions of Bd,
...
that would be severely constraining on a very large part of all Bd code.
...
% need to update this section to use Bd 2 rules for LUB.
...
\LMHash{}%
Bd source text is represented as a sequence of Unicode code points.
...
\LMHash{}%
The text in this appendix is not part of the specification of the Bd language.
...
intended implementation. But when Bd is compiled to JavaScript,
